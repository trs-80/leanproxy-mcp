...
// literal present in the (lowercased) input, the pattern cannot match and its
// scan is skipped. Lowercasing makes the check a superset of case-insensitive
// patterns; a false trigger only costs a regex scan, never a missed secret.
...
// pureLits returns the complete set of (lowercased) strings a node can match,
...
// possible reports whether pattern i can possibly match the lowercased input.
...
// anyPossible reports whether any pattern could match the lowercased input.
