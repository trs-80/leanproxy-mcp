...
| `servers[].tools.include` | list | all | **Per-server** allowlist of upstream tool names to expose. Everything else is dropped from `tools/list` stubs, `search_tools`, and `list_tools`. Every exposed tool is paid for on every turn (schema, plus name echoes on clients like Bob), so trim servers to the tools you actually use. |
