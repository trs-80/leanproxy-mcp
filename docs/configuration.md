...
| `servers[].tools.include` | list | all | **Per-server** allowlist of upstream tool names to expose. Everything else is dropped from `tools/list` stubs, `search_tools`, and `list_tools`, and calls to filtered-out tools are rejected at dispatch. Every exposed tool is paid for on every turn (schema, plus name echoes on clients like Bob), so trim servers to the tools you actually use. |
| `servers[].tools.exclude` | list | none | **Per-server** denylist of tool names, applied after `include`. |
| `servers[].tools.max_response_chars` | map | none | **Per-tool** result cap in characters (`index_status: 1500`). Overrides the global `--max-response-chars` default for that tool; an explicit `max_response_chars` argument overrides both, on `invoke_tool` or inside a direct `serverName_toolName` call's arguments (the proxy strips it before forwarding unless the tool's own schema declares it). Under flat-rate billing an oversized result is re-billed on every later turn. Only cap tools with unbounded prose output; never cap list/paginated results — a truncated list makes the model re-fetch page by page (measured: a 1500-char cap on `list_projects` turned 1 call into 6–12 and tripled the task cost). |
| `servers[].tools.cache_ttl` | map | none | **Per-tool** exact-match result cache TTL (`list_projects: 60s`). A repeated call with the same tool and byte-identical arguments within the TTL is served from memory — no upstream round trip. Opt-in and per tool: list only idempotent read-style tools (`list_*`, `*_status`, doc lookups); a cached result is replayed verbatim until the TTL lapses, so never cache mutating tools. Results with a tool-level `isError` are never cached. Per-call `max_response_chars` still applies on hits (the cache stores the full result). |
| `servers[].tools.adaptive_stub_after` | duration | off | **Per-server** usage-adaptive stubs (`adaptive_stub_after: 168h`). A tool not invoked within the window renders as a name-only stub in lazy `tools/list` — no description, bare object schema — reclaiming its schema tokens on every turn. The tool stays fully callable and searchable (`search_tools`/`get_tool_schema` keep the full data), and a single invocation restores its full stub on the next `tools/list`. Tools with no usage history get a full window from first sight before demotion. Usage recency persists in `~/.config/leanproxy/toolusage.json`. |
| `optimization.minify_results` | bool | `true` | Lossless result minification: pretty-printed JSON inside result text blocks is compacted (`json.Compact` — whitespace only, never reorders keys or rewrites numbers), and a `structuredContent` that byte-duplicates a text block is dropped (the spec has servers mirror the same JSON in both, billing it twice). Set `false` to pass results through untouched. |
